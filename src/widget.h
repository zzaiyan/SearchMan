...
class Widget : public QWidget {
 ...
 private slots:
  ...
  void slotCrawlQuestion(const QString& title,
 ...
 private:
  ...
  void show_results(int* resultPages,
                    int szResultPages,
                    vector<string> targetWords);  //显示交集、对称差
  QString str_to_html(const QString& str,
                      const vector<QString>& keywords,
                      int fileNum,
                      int type);  //将普通字符串转为HTML格式
};
